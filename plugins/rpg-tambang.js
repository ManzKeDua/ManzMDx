...
async function handler(m, { conn, text }) {
    ...
    if (m.sender in conn.tambang) {
        ...
        if (text === 'start') {
            ...
            if (typeof user.resources !== "object") global.db.data.users[m.sender].resources = { diamond: 0, gold: 0, stone: 0, emerald: 0, coal: 0, iron: 0 };
            ...
            let currentArea = 0;
            let hasilTambang = 0;
            let totalReward = { diamond: 0, gold: 0, stone: 0, emerald: 0, coal: 0, iron: 0 };
            ...
            let caption = `🏞️ *AREA PERTAMBANGAN:* ${area}\n\n🪨 Ketik *'${txt}'* untuk memulai pertambangan di area ini.\n🔍 Jumlah hasil tambang yang didapatkan: ${hasilTambang}\n💰 Exp yang didapatkan: ${reward.exp}\n💎 Resources yang didapatkan: Diamond: ${reward.resources.diamond}, Gold: ${reward.resources.gold}, Stone: ${reward.resources.stone}, Emerald: ${reward.resources.emerald}, Coal: ${reward.resources.coal}, Iron: ${reward.resources.iron}`;
    ...
    if (msg === 'stop') {
        ...
        if (areas[currentArea].txt === msg) {
            ...
            if (currentArea >= areas.length) {
                m.reply(`🎉 Selamat! Anda telah menyelesaikan semua area pertambangan.\nTotal hasil tambang: ${hasilTambang}\nExp yang didapatkan: ${reward.exp}\nTotal resources yang didapatkan: Diamond: ${totalReward.diamond}, Gold: ${totalReward.gold}, Stone: ${totalReward.stone}, Emerald: ${totalReward.emerald}, Coal: ${totalReward.coal}, Iron: ${totalReward.iron}`);
                delete conn.tambang[m.sender];
                return false;
            } else {
