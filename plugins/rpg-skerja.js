const jobs = {
    ...
    youtuber: [{
        name: 'Youtuber',
        task: '📱 Membuat konten video youtube',
        difficulty: pickRandom(['Noob', 'Easy', 'Normal']),
        money: randomMoney(50000, 1),
        exp: 30
    }],
...
const taskInformation = {
    ...
    '🛒 Mencari pembeli': [
     ...
     '📱 Membuat konten video youtube': [
        'Membuat konten mukbang berbagai macam makanan dan minuman',
        'Membuat konten vlog bepergian kesebuah tempat dan luar negara.',
        'Membuat konten bermain video game.',
        'Membuat konten riview berbagai gadget dan memasarkan sebuah gadget.',
        'Membuat konten memasak dan membagikan resep-resep makanan.'
    ],
    '💉 Merawat pasien': [
...
// Export handler
handler.command = /^skerja$/i;
module.exports = handler;

// Helper functions (unchanged)
function randomMoney(max, min) {
    const maxSalary = 3000000; // Maximum salary limit (3 juta)
    const randomSalary = Math.floor(Math.random() * (max - min + 1)) + min;
    return Math.min(randomSalary, maxSalary); // Return the lesser of randomSalary and maxSalary
}
