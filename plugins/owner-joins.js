...
const handler = async (m, { conn, text, args }) => {
    ...
    try {
        ...
        if (e.length) await conn.reply(res, `Ada @${anubot} Owner-ku Di Sini, Aku Mau Keluar Aja Dah, Takut Kena Marah.\n@${conn.user.jid.split(`@`)[0]} akan keluar 5 detik lagi\nBye😑\nThanks dah invite Gua *${m.name}*`, fkonn, {
            ...
            await conn.reply(anubot + '@s.whatsapp.net', `*INVITING!*\n\n@${m.sender.split('@')[0]} telah mengundang ${conn.user.name} ke grup\n\n${await conn.getName(res)}\n\n${res}\n\nPesan : ${args[0]}\n\nBot akan keluar otomatis setelah *${msToDate(global.db.data.chats[res].expired - now)}*`, null, {
                mentions: [m.sender]
            });
        });

        if (!e.length) await conn.reply(anubot + '@s.whatsapp.net', `*INVITING!*\n\n@${m.sender.split('@')[0]} telah mengundang ${conn.user.name} ke grup\n\n${await conn.getName(res)}\n\n${res}\n\nPesan : ${args[0]}\n\nBot akan keluar otomatis setelah *${msToDate(global.db.data.chats[res].expired - now)}*`, null, {
            mentions: [m.sender]
        });
        ...
        conn.reply(anubot + '@s.whatsapp.net', e);
