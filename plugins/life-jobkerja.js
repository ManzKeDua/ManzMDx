...
let handler = async (m, { isPrems, conn, text, usedPrefix, command }) => {
    ...
    if (new Date() - user.lastjobkerja < cooldown || user.lastjobkerja + cooldownAfterWork > new Date()) {
        let remainingTime;
        if (new Date() - user.lastjobkerja < cooldown) {
            remainingTime = user.lastjobkerja + cooldown - new Date();
        } else {
            remainingTime = user.lastjobkerja + cooldownAfterWork - new Date();
        }
    ...
    if (jobList[user.job]) {
        ...
        user.bank += bank;
        user.jobexp += 1;
        user.lastjobkerja = new Date().getTime();
