        ...
        if (/craft|Crafting|blacksmith/i.test(command)) {
            ...
            switch (type) {
                ...
                case 'sword':
                    ...
                    if (user.batu < 10 || user.kayu < 15 || user.iron < 15 || user.string < 10) return m.reply(`Barang tidak cukup!\nUntuk membuat axe. Kamu memerlukan : ${user.kayu < 15 ? `\n${15 - user.kayu} kayu🪵` : ''} ${user.iron < 15 ? `\n${15 - user.iron} iron⛓` : ''}${user.string < 10 ? `\n${10 - user.string} String🕸️` : ''}${user.batu < 10 ? `\n${10 - user.batu} Batu 🪨` : ''}`);
                    user.kayu -= 15;
                    user.iron -= 15;
                    user.batu -= 10;
                    user.string -= 10;
